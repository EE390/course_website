...
(function (global) {
  ...
  /* ASEM-51 expressions. Precedence, loosest first:
   *   OR XOR | ^   <   AND &   <   + -   <   * / MOD SHL SHR   <   NOT HIGH LOW ( )
   */
  function evalExpr(src, ctx) {
    ...
    function word(w) { return new RegExp("^" + w + "\\b", "i").test(s.slice(pos)); }
    ...
    function primary() {
      ...
      if (s[pos] === "+") { pos++; return primary(); }
      if (word("NOT")) { pos += 3; return ~primary(); }
      if (word("HIGH")) { pos += 4; return (primary() >> 8) & 0xFF; }
      if (word("LOW")) { pos += 3; return primary() & 0xFF; }
      var start = pos;
    ...
    function term() {
      ...
      for (;;) {
        ...
        else if (word("MOD")) { pos += 3; v = v % primary(); }
        else if (word("SHL")) { pos += 3; v = v << primary(); }
        else if (word("SHR")) { pos += 3; v = v >> primary(); }
        else return v;
      }
    }
    ...
    function conj() {
      ...
      for (;;) {
        skip();
        if (s[pos] === "&") { pos++; v = v & sum(); }
        else if (word("AND")) { pos += 3; v = v & sum(); }
        else return v;
      }
    }
    ...
    function expr() {
      ...
      for (;;) {
        skip();
        if (s[pos] === "|") { pos++; v = v | conj(); }
        else if (s[pos] === "^") { pos++; v = v ^ conj(); }
        else if (word("XOR")) { pos += 3; v = v ^ conj(); }
        else if (word("OR")) { pos += 2; v = v | conj(); }
        else return v;
      }
    }
   ...
   */
  /* How a number is shown back to the student, in the base they most likely wrote. */
  function show(v) {
    if (v < 0) return String(v);
    return v > 9 ? v.toString(16).toUpperCase() + "H" : String(v);
  }

  /* Values are only range-checked in pass 2, once every forward reference is known.
   * Until then a symbol reads as 0 and any check would be meaningless. */
  function imm(text, ctx) {
    var v = evalExpr(text, ctx);
    if (ctx.pass === 2 && (v < -256 || v > 255)) {
      throw new AsmError(show(v) + " does not fit in one byte" +
        " (write LOW(" + show(v) + ") if you meant its low half)", ctx.line);
    }
    return v & 0xFF;
  }

  function imm16(text, ctx) {
    var v = evalExpr(text, ctx);
    if (ctx.pass === 2 && (v < -0x10000 || v > 0xFFFF)) {
      throw new AsmError(show(v) + " does not fit in two bytes", ctx.line);
    }
    return v & 0xFFFF;
  }

  function dir(text, ctx) {
    var v = evalExpr(text, ctx);
    if (ctx.pass === 2 && (v < 0 || v > 255)) {
      throw new AsmError("address " + show(v) + " is outside 00H..0FFH;" +
        " only the 128 bytes of RAM and the SFRs can be addressed directly", ctx.line);
    }
    return v & 0xFF;
  }

  /* Bit operands. A bare register name is a byte, not a bit, so SETB P1 is the
   * mistake it looks like rather than a silent SETB P1.0. */
  function bitAddr(text, ctx) {
    var up = String(text).trim().toUpperCase();
    if (SFR.hasOwnProperty(up) ||
        (ctx.symbols.hasOwnProperty(up) && ctx.symbols[up].kind === "data")) {
      throw new AsmError("'" + String(text).trim() + "' is a register, not a bit;" +
        " write " + String(text).trim() + ".0 to " + String(text).trim() + ".7 for one of its bits", ctx.line);
    }
    var v = evalExpr(text, ctx);
    if (ctx.pass === 2 && (v < 0 || v > 255)) {
      throw new AsmError("bit address " + show(v) + " is outside 00H..0FFH", ctx.line);
    }
    return v & 0xFF;
  }
  ...
  function family(mn, immOp, dirOp, iriOp, regOp) {
    OPS[mn] = function (ops, ctx) {
      ...
      if (a.k === "A") {
        if (b.k === "#") return [immOp, imm(b.expr, ctx)];
        if (b.k === "R") return [regOp + b.n];
        if (b.k === "@R") return [iriOp + b.n];
        if (b.k === "direct") return [dirOp, dir(b.expr, ctx)];
      }
  ...
  function logic(mn, immOp, dirOp, iriOp, regOp, dirAOp, dirImmOp, cBitOp, cNotBitOp) {
    OPS[mn] = function (ops, ctx) {
      ...
      if (a.k === "C" && cBitOp !== undefined) {
        if (b.k === "/bit") return [cNotBitOp, bitAddr(b.expr, ctx)];
        return [cBitOp, bitAddr(b.expr, ctx)];
      }
      if (a.k === "direct") {
        if (b.k === "A") return [dirAOp, dir(a.expr, ctx)];
        if (b.k === "#") return [dirImmOp, dir(a.expr, ctx), imm(b.expr, ctx)];
      }
      throw new AsmError(mn + " cannot take these operands", ctx.line);
    };
  }
  ...
  OPS.MOV = function (ops, ctx) {
    var a = operandKind(ops[0]), b = operandKind(ops[1]);
    if (a.k === "A") {
      if (b.k === "#") return [0x74, imm(b.expr, ctx)];
      if (b.k === "R") return [0xE8 + b.n];
      if (b.k === "@R") return [0xE6 + b.n];
      if (b.k === "direct") return [0xE5, dir(b.expr, ctx)];
    }
    if (a.k === "R") {
      if (b.k === "A") return [0xF8 + a.n];
      if (b.k === "#") return [0x78 + a.n, imm(b.expr, ctx)];
      if (b.k === "direct") return [0xA8 + a.n, dir(b.expr, ctx)];
    }
    if (a.k === "@R") {
      if (b.k === "A") return [0xF6 + a.n];
      if (b.k === "#") return [0x76 + a.n, imm(b.expr, ctx)];
      if (b.k === "direct") return [0xA6 + a.n, dir(b.expr, ctx)];
    }
    if (a.k === "DPTR" && b.k === "#") {
      var v = imm16(b.expr, ctx);
      return [0x90, (v >> 8) & 0xFF, v & 0xFF];
    }
    if (a.k === "C" && b.k === "direct") return [0xA2, bitAddr(b.expr, ctx)];
    if (a.k === "direct" && b.k === "C") return [0x92, bitAddr(a.expr, ctx)];
    if (a.k === "direct") {
      if (b.k === "A") return [0xF5, dir(a.expr, ctx)];
      if (b.k === "R") return [0x88 + b.n, dir(a.expr, ctx)];
      if (b.k === "@R") return [0x86 + b.n, dir(a.expr, ctx)];
      if (b.k === "#") return [0x75, dir(a.expr, ctx), imm(b.expr, ctx)];
      if (b.k === "direct") return [0x85, dir(b.expr, ctx), dir(a.expr, ctx)];
    }
  ...
  function incdec(mn, aOp, dirOp, iriOp, regOp, dptrOp) {
    OPS[mn] = function (ops, ctx) {
      ...
      if (a.k === "direct") return [dirOp, dir(a.expr, ctx)];
  ...
  function bitop(mn, aOp, cOp, bitOp) {
    OPS[mn] = function (ops, ctx) {
      ...
      if (a.k === "C") return [cOp];
      if (a.k === "direct") return [bitOp, bitAddr(a.expr, ctx)];
      throw new AsmError(mn + " cannot take this operand", ctx.line);
    };
  }
  ...
  OPS.PUSH = function (ops, ctx) { return [0xC0, dir(operandKind(ops[0]).expr, ctx)]; };
  OPS.POP = function (ops, ctx) { return [0xD0, dir(operandKind(ops[0]).expr, ctx)]; };

  OPS.XCH = function (ops, ctx) {
    var a = operandKind(ops[0]), b = operandKind(ops[1]);
    if (a.k !== "A") throw new AsmError("XCH always exchanges with A: write XCH A," + ops[0], ctx.line);
    if (b.k === "R") return [0xC8 + b.n];
    if (b.k === "@R") return [0xC6 + b.n];
    if (b.k === "direct") return [0xC5, dir(b.expr, ctx)];
    throw new AsmError("XCH cannot take these operands", ctx.line);
  };
  OPS.XCHD = function (ops, ctx) {
    var a = operandKind(ops[0]), b = operandKind(ops[1]);
    if (a.k === "A" && b.k === "@R") return [0xD6 + b.n];
    throw new AsmError("XCHD needs A,@Ri", ctx.line);
  };

  OPS.LJMP = function (ops, ctx) {
    var v = imm16(ops[0], ctx);
  ...
  function bitRel(mn, opcode) {
    OPS[mn] = function (ops, ctx) {
      return [opcode, bitAddr(ops[0], ctx), rel(evalExpr(ops[1], ctx), ctx.pc + 3, ctx)];
    };
  }
  ...
  OPS.DJNZ = function (ops, ctx) {
    ...
    if (a.k === "direct") {
      return [0xD5, dir(a.expr, ctx), rel(evalExpr(ops[1], ctx), ctx.pc + 3, ctx)];
    }
  ...
  OPS.CJNE = function (ops, ctx) {
    ...
    var target = ops[2];
    if (a.k === "A" && b.k === "#") {
      return [0xB4, imm(b.expr, ctx), rel(evalExpr(target, ctx), ctx.pc + 3, ctx)];
    }
    if (a.k === "A" && b.k === "direct") {
      return [0xB5, dir(b.expr, ctx), rel(evalExpr(target, ctx), ctx.pc + 3, ctx)];
    }
    if (a.k === "R" && b.k === "#") {
      return [0xB8 + a.n, imm(b.expr, ctx), rel(evalExpr(target, ctx), ctx.pc + 3, ctx)];
    }
    if (a.k === "@R" && b.k === "#") {
      return [0xB6 + a.n, imm(b.expr, ctx), rel(evalExpr(target, ctx), ctx.pc + 3, ctx)];
    }
  ...
  function dataBytes(operandText, ctx, wordSize) {
    ...
    splitOperands(operandText).forEach(function (item) {
      ...
      if (wordSize === 2) {
        var w = imm16(t, ctx);
        out.push((w >> 8) & 0xFF); out.push(w & 0xFF);
      } else {
        out.push(imm(t, ctx));
      }
    });
  ...
  function assemble(source) {
    ...
    /* Reported straight away rather than thrown: a pass-1 throw is swallowed,
     * because forward references make most pass-1 complaints meaningless, and
     * pass 2 rebinds labels without re-checking. */
    function redefined(name, rec) {
      errors.push({
        line: rec.n,
        message: "'" + name + "' is defined more than once (first at line " +
          symbols[name].line + ")",
        text: rec.raw
      });
    }
        ...
        try {
          ...
          if (mn === "EQU" || mn === "SET" || mn === "DATA" || mn === "BIT" ||
            ...
            if (pass === 1 && symbols.hasOwnProperty(rec.label) && mn !== "SET") {
              redefined(rec.label, rec);
              continue;
            }
            symbols[rec.label] = { value: val, kind: mn.toLowerCase(), line: rec.n };
            continue;
          }
          if (rec.label) {
            if (pass === 1 && symbols.hasOwnProperty(rec.label)) {
              redefined(rec.label, rec);
            } else {
              symbols[rec.label] = { value: pc, kind: "label", line: rec.n };
            }
          }
          if (!mn) continue;
